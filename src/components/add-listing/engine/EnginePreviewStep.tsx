...
interface EnginePreviewStepProps {
  formData: {
    brand: string;
    model: string;
    engineType: string;
...
export function EnginePreviewStep({ formData, onBack, onClose, onPublish }: EnginePreviewStepProps) {
  ...
  const hasSpecs = !!(formData.engineType || formData.displacement || formData.power || formData.cylinderLayout || formData.cylinderCount);
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-background">
      ...
      <div className="mx-auto w-full max-w-[720px] flex-1 px-5 pt-6 pb-[calc(160px+env(safe-area-inset-bottom,0px))]">
        ...
        {/* Engine Specs */}
        {hasSpecs && (
          <Section title="Характеристики двигателя" isExpanded={expandedSections.specs} onToggle={() => toggleSection("specs")}>
            {formData.engineType ? <InfoRow label="Тип двигателя" value={formData.engineType} /> : null}
            {formData.displacement ? <InfoRow label="Объём двигателя" value={`${formData.displacement} см³`} /> : null}
            ...
            {formData.cylinderCount ? <InfoRow label="Количество цилиндров" value={formData.cylinderCount} /> : null}
          </Section>
        )}

        {/* Condition */}
        {formData.condition ? (
          <Section title="Состояние" isExpanded={expandedSections.condition} onToggle={() => toggleSection("condition")}>
            <p className="whitespace-pre-wrap text-[14px] font-normal text-black">{formData.condition}</p>
          </Section>
        ) : null}
