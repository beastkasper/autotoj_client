...
export function ExitConfirmationModal({ isOpen, onStay, onExit }: ExitConfirmationModalProps) {
  // onRequestClose → Esc
  useEffect(() => {
    if (!isOpen) return;
    ...
    >
      ...
      >
        ...
        <div className="flex flex-row">
          ...
          <button type="button" onClick={onExit} className="flex flex-1 items-center justify-center py-3.5">
            <span className="text-[17px] font-semibold text-[#D32F2F]">Выйти</span>
          </button>
        </div>
      </div>
    </div>,
