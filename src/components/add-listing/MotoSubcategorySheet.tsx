...
export function MotoSubcategorySheet({
  onSelect,
  onBack,
  onClose,
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  hasUnsavedData = false,
}: MotoSubcategorySheetProps) {
  const subcategories = ["Мотоциклы", "Мотовездеходы", "Снегоходы", "Скутеры"];
      ...
      >
        <div className="relative mx-auto flex h-14 w-full max-w-[720px] flex-row items-center justify-between px-4">
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-0 flex h-14 items-center justify-center">
            <span className="text-[17px] font-semibold text-foreground">Мото</span>
          </div>
