...
interface StepPowerProps {
  form: CarListingForm;
  onUpdate: <K extends keyof CarListingForm>(key: K, value: CarListingForm[K]) => void;
}

export function StepPower({ form, onUpdate }: StepPowerProps) {
  return (
    <div className="flex flex-col gap-5 p-4">
      ...
      <div className="relative">
        <input
          ...
          onChange={(e) => onUpdate("enginePower", e.target.value.replace(/\D/g, ""))}
          placeholder="150"
          className={cn(
            "w-full h-12 px-4 pr-12 rounded-[14px] border border-[#D0D0D0] text-[16px] font-[family-name:var(--font-manrope)] outline-none transition-colors focus:border-black"
          )}
        />
